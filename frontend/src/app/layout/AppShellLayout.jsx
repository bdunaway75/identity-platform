...
export default function AppShellLayout() {
  ...
  const { tier, navItems, isLoggedIn } = useNavBar();
  const navItemList = useMemo(() => Array.from(navItems.values()), [navItems]);
  ...
  const currentNavItem = navItemList.find((item) => item.matches(currentPath)) ?? navItemList[0];

  //Close mobile menu on re-renders
  useEffect(() => {
  ...
  const authActionLabel = isLoggedIn
    ? (isAuthActionPending ? "Signing out..." : "Log out")
    : (isAuthActionPending ? "Logging In..." : "Login");
          ...
          >
            <div className="mobile-nav-panel">
              ...
              <nav className="mobile-nav-list">
                {renderNavLinks("mobile-nav-item")}
                <button
                  type="button"
                  className="mobile-nav-item mobile-nav-auth-item"
                  onClick={handleAuthAction}
                  disabled={isAuthActionPending}
                >
                  {authActionLabel}
